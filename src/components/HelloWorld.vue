...
class Game {
  ...
  play(song:Song){
    ...
    },song.startTime - this.getBpmTime()*1.8 - 1001)
...
<template>
  <div class="h-full w-full bg-black" >
    <video ref="video" :src="src" class="h-full w-full" webkit-playsinline="true"
    playsinline="true"
    x5-video-player-type='h5-page'></video>
    <Transition name="box">
      <div v-show="_game.show" class="keyboard absolute bottom-[30%] left-0 right-0 bg-[#00000088] h-[60px] w-[465px] rounded-full m-auto">
        <div class="inputbox h-full w-full  border-solid border-2 border-black rounded-full">
          <div class="flex h-full w-full justify-center items-center border-solid border border-white rounded-full">
            <div v-for="(item, index) of _game.nowForm" :key="index" :class="{isOk: index<_game.checkIndex, arrow0: item == 0, arrow1: item == 1, arrow2: item == 2, arrow3: item == 3}">
              <div class="border border-white rounded-full m-1 bg-gradient-to-r from-cyan-500 to-blue-500" :class="{'from-green-500':index<_game.checkIndex, 'to-green-600':index<_game.checkIndex}">
                <svg t="1734621948561" class="icon h-[40px] w-[40px]" viewBox="100 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="22616" id="mx_n_1734621948561" data-spm-anchor-id="a313x.search_index.0.i10.6e8f3a81Jbme8S" xmlns:xlink="http://www.w3.org/1999/xlink" width="300" height="300">
                  <g>
                    <path d="M529.066667 162.133333c-25.6 34.133333-17.066667 76.8 8.533333 110.933334L691.2 426.666667H300c-42.667 0-85.333333 34.133333-85.333333 85.333333v8.533333c8.533333 42.666667 42.666667 76.8 85.333333 76.8h403.2L546.133333 750.933333c-34.133333 34.133333-34.133333 85.333333 0 119.466667s85.333333 34.133333 119.466667 0l290.133333-298.666667c34.133333-34.133333 34.133333-85.333333 0-119.466666L657.066667 153.6c-34.133333-34.133333-85.333333-34.133333-128 8.533333z" p-id="22617" fill="#ffffff" data-spm-anchor-id="a313x.search_index.0.i9.6e8f3a81Jbme8S" class="selected">
                    </path>
                  </g>
                </svg>
              </div>
...
.jindu .flash{
  ...
  animation-name: scaleXAnimation; /* 引用动画名称 */
  animation-duration: 0.4s; /* 动画持续时间 */
  animation-iteration-count: infinite; /* 无限循环动画 */
  animation-timing-function: ease-in-out; /* 动画的缓动函数，使动画更平滑 */
}
@keyframes scaleXAnimation {
  0%, 100% {
    transform: scaleX(1); /* 动画开始和结束时的X轴缩放比例为1 */
  }
  50% {
    transform: scaleX(0.5); /* 动画中间时的X轴缩放比例为2 */
  }
}
</style>
